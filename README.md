...
```sh
curl -sSfL https://raw.githubusercontent.com/lasthyphen/dijets-network-runner/main/scripts/install.sh | sh -s
```

To install a specific version, just append the desired version to the command (must be an existing github tag like v1.3.1)

```sh
curl -sSfL https://raw.githubusercontent.com/lasthyphen/dijets-network-runner/main/scripts/install.sh | sh -s v1.3.1
```
...
This is only needed by advanced users who want to modify or test Dijets Network Runner in specific ways.
...
```sh
git clone https://github.com/lasthyphen/dijets-network-runner.git
```
...
The E2E test checks `dijets-network-runner` RPC communication and control. It starts a network against a fresh RPC
...
## Using `dijets-network-runner`

You can import this repository as a library in your Go program, but we recommend running `dijets-network-runner` as a binary. This creates an RPC server that you can send requests to in order to start a network, add nodes to the network, remove nodes from the network, restart nodes, etc.. You can make requests through the `dijets-network-runner` command or by making API calls. Requests are "translated" into gRPC and sent to the server.

**Why does `dijets-network-runner` need an RPC server?** `dijets-network-runner` needs to provide complex workflows such as replacing nodes, restarting nodes, injecting fail points, etc.. The RPC server exposes basic operations to enable a separation of concerns such that one team develops a test framework, and the other writes test cases and controlling logic.
...
```bash
# replace execPath with the path to dijetsnode on your machine
# e.g., ${HOME}/go/src/github.com/lasthyphen/dijetsnode/build/dijetsnode
DIJETSNODE_EXEC_PATH="dijetsnode"

curl -X POST -k http://localhost:8081/v1/control/start -d '{"execPath":"'${DIJETSNODE_EXEC_PATH}'","numNodes":5,"logLevel":"INFO"}'

# or
dijets-network-runner control start \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--number-of-nodes=5 \
--dijetsnode-path ${DIJETSNODE_EXEC_PATH}
```

Additional optional parameters which can be passed to the start command:

```bash
--plugin-dir ${DIJETSNODE_PLUGIN_PATH} \
...
For example, to set `dijetsnode --http-host` flag for all nodes:

```bash
# to expose local RPC server to all traffic
# (e.g., run network runner within cloud instance)
curl -X POST -k http://localhost:8081/v1/control/start -d '{"execPath":"'${DIJETSNODE_EXEC_PATH}'","globalNodeConfig":"{\"http-host\":\"0.0.0.0\"}"}'
...
The network-runner supports dijetsnode node configuration at different levels.
...
2. `--global-node-config` is a JSON string representing a _single_ dijetsnode config, which will be applied to **all nodes**. This makes it easy to define common properties to all nodes. Whatever is set here will be _combined_ with the standard set above.
...
'{"execPath":"'${DIJETSNODE_EXEC_PATH}'","customNodeConfigs":
{
...
# or
dijets-network-runner control health \
--log-level debug \
--endpoint="0.0.0.0:8080"
```
...
# or
dijets-network-runner control uris \
--log-level debug \
--endpoint="0.0.0.0:8080"
```
...
# or
dijets-network-runner control status \
--log-level debug \
--endpoint="0.0.0.0:8080"
```
...
# or
dijets-network-runner control save-snapshot snapshotName
```
...
# or
dijets-network-runner control load-snapshot snapshotName
```

An dijetsnode binary path and/or plugin dir can be specified when loading the snapshot. This is
optional. If not specified, will use the paths saved with the snapshot:

```bash
curl -X POST -k http://localhost:8081/v1/control/loadsnapshot -d '{"snapshot_name":"node5","execPath":"'${DIJETSNODE_EXEC_PATH}'","pluginDir":"'${DIJETSNODE_PLUGIN_PATH}'"}'

# or
dijets-network-runner control load-snapshot snapshotName --dijetsnode-path ${DIJETSNODE_EXEC_PATH} --plugin-dir ${DIJETSNODE_PLUGIN_PATH}
```
...
# or
dijets-network-runner control get-snapshot-names
```
...
# or
dijets-network-runner control remove-snapshot snapshotName
```
...
# or
dijets-network-runner control create-subnets 5
```
...
# or
dijets-network-runner control create-blockchains '[{"vm_name":"'$VM_NAME'","genesis":"'$GENESIS_PATH'"}]' --plugin-dir $PLUGIN_DIR
```
...
# or
dijets-network-runner control create-blockchains '[{"vm_name":"'$VM_NAME'","genesis":"'$GENESIS_PATH'", "subnet_id": "'$SUBNET_ID'"}]' --plugin-dir $PLUGIN_DIR
```
...
# or
dijets-network-runner control create-blockchains '[{"vm_name":"'$VM_NAME'","genesis":"'$GENESIS_PATH'", "subnet_id": "'$SUBNET_ID'", "chain_config": "'$CHAIN_CONFIG_PATH'", "network_upgrade": "'$NETWORK_UPGRADE_PATH'", "subnet_config": "'$SUBNET_CONFIG_PATH'"}]' --plugin-dir $PLUGIN_DIR
```
...
# or
dijets-network-runner control remove-node \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--node-name node5
...
curl -X POST -k http://localhost:8081/v1/control/restartnode -d '{"name":"node1","execPath":"'${DIJETSNODE_EXEC_PATH}'","logLevel":"INFO"}'

# or
dijets-network-runner control restart-node \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--node-name node1 \
...
# a different execPath
curl -X POST -k http://localhost:8081/v1/control/addnode -d '{"name":"node99","execPath":"'${DIJETSNODE_EXEC_PATH}'","logLevel":"INFO"}'

# or
dijets-network-runner control add-node \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--node-name node99 \
...
`--node-config` allows to specify specific dijetsnode config parameters to the new node. See [here](https://docs.dijetsblockchain.org/build/references/dijetsnode-config-flags) for the reference of supported flags.
...
dijetsnode exposes a "test peer", which you can attach to a node.
(See [here](https://github.com/lasthyphen/dijetsnode/blob/master/network/peer/test_peer.go) for more information.)
...
# or
dijets-network-runner control attach-peer \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--node-name node1
...
# or
dijets-network-runner control send-outbound-message \
--request-timeout=3m \
--log-level debug \
--endpoint="0.0.0.0:8080" \
--node-name node1 \
...
# or
dijets-network-runner control stop \
--log-level debug \
--endpoint="0.0.0.0:8080"
```
...
```bash
# or download from https://github.com/lasthyphensubnet-cli/releases
cd ${HOME}/go/src/github.com/lasthyphensubnet-cli
go install -v .
subnet-cli create VMID subnetevm
# srEXiWaHuhNyGwPUi444Tu47ZEDwxTWrbQiuD7FmgSAQ6X7Dy

# download from https://github.com/lasthyphen/dijetsnode/releases
# or build
rm -rf ${HOME}/go/src/github.com/lasthyphen/dijetsnode/build
cd ${HOME}/go/src/github.com/lasthyphen/dijetsnode
./scripts/build.sh

# ref. https://github.com/lasthyphensubnet-evm/blob/b69e47e0398b5237cda0422f6a32969e64bde346/scripts/run.sh
cd ${HOME}/go/src/github.com/lasthyphensubnet-evm
go build -v \
-o ${HOME}/go/src/github.com/lasthyphen/dijetsnode/build/plugins/srEXiWaHuhNyGwPUi444Tu47ZEDwxTWrbQiuD7FmgSAQ6X7Dy \
./plugin

# make sure binaries are built
find ${HOME}/go/src/github.com/lasthyphen/dijetsnode/build
# for example
# .../build
# .../build/plugins
...
```bash
# replace execPath with the path to dijetsnode on your machine
DIJETSNODE_EXEC_PATH="${HOME}/go/src/github.com/lasthyphen/dijetsnode/build/dijetsnode"
DIJETSNODE_PLUGIN_PATH="${HOME}/go/src/github.com/lasthyphen/dijetsnode/build/plugins"

curl -X POST -k http://localhost:8081/v1/control/start -d '{"execPath":"'${DIJETSNODE_EXEC_PATH}'","numNodes":5,"logLevel":"INFO","pluginDir":"'${DIJETSNODE_PLUGIN_PATH}'","blockchainSpecs":[{"vm_name":"subnetevm","genesis":"/tmp/subnet-evm.genesis.json"}]}'
...
```bash
curl -X POST -k http://localhost:8081/v1/control/start -d '{"execPath":"'${DIJETSNODE_EXEC_PATH}'","numNodes":5,"logLevel":"INFO","pluginDir":"'${DIJETSNODE_PLUGIN_PATH}'","blockchainSpecs":[{"vm_name":"subnetevm","genesis":"/tmp/subnet-evm.genesis.json","chain_config":"'$CHAIN_CONFIG_PATH'","network_upgrade":"'$NETWORK_UPGRADE_PATH'","subnet_config":"'$SUBNET_CONFIG_PATH'"}]}'
...
cd ${HOME}/go/src/github.com/lasthyphenblobvm
go build -v \
-o ${HOME}/go/src/github.com/lasthyphen/dijetsnode/build/plugins/kM6h4LYe3AcEU1MB2UNg6ubzAiDAALZzpVrbX8zn3hXF6Avd8 \
./cmd/blobvm
...
curl -X POST -k http://localhost:8081/v1/control/start -d '{"execPath":"'${DIJETSNODE_EXEC_PATH}'","numNodes":5,"logLevel":"INFO","pluginDir":"'${DIJETSNODE_PLUGIN_PATH}'","blockchainSpecs":[{"vm_name":"blobvm","genesis":"/tmp/blobvm.genesis.json"}]}'
...
```bash
curl -X POST -k http://localhost:8081/v1/control/start -d '{"execPath":"'${DIJETSNODE_EXEC_PATH}'","numNodes":5,"logLevel":"INFO","pluginDir":"'${DIJETSNODE_PLUGIN_PATH}'","blockchainSpecs":[{"vm_name":"blobvm","genesis":"/tmp/blobvm.json","chain_config":"'$CHAIN_CONFIG_PATH'","network_upgrade":"'$NETWORK_UPGRADE_PATH'","subnet_config":"'$SUBNET_CONFIG_PATH'"}]}'
...
# or download from https://github.com/lasthyphentimestampvm/releases
# cd ${HOME}/go/src/github.com/lasthyphentimestampvm
# ./scripts/build.sh
cd ${HOME}/go/src/github.com/lasthyphentimestampvm
go build -v \
-o ${HOME}/go/src/github.com/lasthyphen/dijetsnode/build/plugins/tGas3T58KzdjcJ2iKSyiYsWiqYctRXaPTqBCA11BqEkNg8kPc \
./main
...
curl -X POST -k http://localhost:8081/v1/control/start -d '{"execPath":"'${DIJETSNODE_EXEC_PATH}'","numNodes":5,"logLevel":"INFO","pluginDir":"'${DIJETSNODE_PLUGIN_PATH}'","blockchainSpecs":[{"vmName":"timestampvm","genesis":"/tmp/timestampvm.genesis.json","blockchain_alias":"timestamp"}]}'
...
You can create a custom dijetsnode genesis with function `network.NewDijetsNodeGenesis`:
...
The network runner allows users to interact with an dijetsnode network using the `network.Network` interface:
...
```go
// Node represents an dijetsnode node
type Node interface {
  ...
  // Return this node's dijetsnode binary path
